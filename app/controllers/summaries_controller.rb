class SummariesController < ApplicationController
  ...
  def visa
    ...
    total_amount = sprintf("%.02f", alumnos.sum("amount").to_s).rjust(15, "0")
    ...
    File.open("text.txt", "w") do |f|
      ...
      alumnos.each do |alumno|
        alumno_line = alumno_register_type+alumno.card_number.to_s.rjust(16, "0")+alumno_reserved_space_1+alumno.bill.to_s.rjust(8, "0")+
                      date+transaction_code+sprintf("%.02f", alumno.amount.to_s).rjust(15, "0")+alumno.identifier.to_s.rjust(15, "0")+
